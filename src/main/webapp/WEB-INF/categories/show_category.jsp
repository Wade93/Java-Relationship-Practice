	...
	<div class="container">
		...
		<div class="row">
			<div class="col border">
				...
				<ul>
				<c:forEach items="${catProducts}" var="catProduct">
					<li>${catProduct.name}</li>
				</c:forEach>
				</ul>
			</div>
			<div class="col border">
				...
				<form:form action="/categories/${category.id}" method="post" modelAttribute="category">
					...
					<select name="productID">
						<c:forEach items="${availableProducts}" var="otherProduct">
							<option value="${otherProduct.id}">${otherProduct.id} ${otherProduct.name}</option>
						</c:forEach>
					</select>
